...
async function loadCreditosCorpModule() {
    ...
    try {
        ...
        console.error(e);
        document.getElementById('main-content').innerHTML = '<p class="text-red-500 p-4">Error al cargar la cartera de créditos.</p>';
    }
}
                ...
                <table class="w-full text-sm text-left">
                    <thead class="bg-gray-100 text-gray-600 uppercase">
                        <tr>
                            ...
                            <th class="p-3 text-right">Saldo Plan</th> <!-- Título actualizado -->
...
function renderFilas(creditos) {
    ...
    return creditos.map(c => {
        // CAMBIO AQUÍ: Usamos saldo_plan si existe, sino saldo_capital
        // saldo_plan viene de la consulta SQL actualizada
        const saldoMostrar = c.saldo_plan ? c.saldo_plan : c.saldo_capital;
        ...
        <tr class="hover:bg-gray-50 border-b">
            ...
            <td class="p-3 text-right font-bold text-blue-800">${formatCurrency(saldoMostrar)}</td>
...
async function cargarGarantiasCliente(clienteId) {
    ...
    try {
        ...
        container.innerHTML = propias.map(g => {
            const estaEnUso = parseInt(g.en_uso_activo) > 0;
            const disabledAttr = estaEnUso ? 'disabled' : '';
            const cursorClass = estaEnUso ? 'cursor-not-allowed bg-gray-100 opacity-60' : 'cursor-pointer hover:bg-white';
            const badgeUso = estaEnUso ? `<span class="ml-2 px-2 py-0.5 bg-red-100 text-red-700 text-xs font-bold rounded border border-red-200"><i class="fas fa-lock mr-1"></i>En Uso</span>` : '';

            return `
                <label class="flex items-center justify-between p-3 border-b transition-colors ${cursorClass}">
                    <div class="flex items-center gap-3">
                        <input type="checkbox" name="garantias_check" value="${g.id}" 
                               ...
                               class="form-checkbox h-5 w-5 text-blue-600 rounded" ${disabledAttr}>
                        <div>
                            <p class="font-bold text-gray-800 text-sm flex items-center">${g.descripcion_bien} ${badgeUso}</p>
                            <p class="text-xs text-gray-500">${g.tipo_nombre} • Avalúo: ${formatCurrency(g.valor_comercial)}</p>
                        </div>
                    </div>
                    <div class="text-right">
                        <span class="block text-xs font-bold text-gray-500 uppercase">Valor Realización</span>
                        <span class="block text-sm font-bold text-green-600">${formatCurrency(g.valor_realizacion)}</span>
                    </div>
                </label>
            `;
        }).join('');
    } catch(e) { container.innerHTML = '<p class="text-red-500 text-center">Error al cargar garantías.</p>'; }
}
...
// ==========================================
// 5. MÓDULO DE COBRANZA (VALIDACIÓN Y LIQUIDACIÓN)
// ==========================================
...
async function showFormularioPago(creditoId) {
    ...
    try {
        ...
        // 1. Configurar Fecha Sugerida (Vencimiento cuota)
        let fechaSugerida = hoy;
        if(data.proxima_cuota_plan && data.proxima_cuota_plan.fecha_vencimiento) {
             fechaSugerida = data.proxima_cuota_plan.fecha_vencimiento;
        }

        // 2. Configurar Monto Sugerido (Cuota Total)
        let montoSugerido = "0.00";
        if(data.proxima_cuota_plan && data.proxima_cuota_plan.cuota_total) {
             montoSugerido = data.proxima_cuota_plan.cuota_total;
        }

        // 3. Obtener comisión pendiente si existe
        let comisionPendiente = 0;
        if(data.proxima_cuota_plan && data.proxima_cuota_plan.comision_programada) {
            comisionPendiente = parseFloat(data.proxima_cuota_plan.comision_programada);
        }
        
        // 4. CALCULO CORRECTO DEL SALDO ACTUAL BASADO EN PLAN
        // Usamos el saldo proyectado de la cuota pendiente + su capital (porque saldo_proyectado es el remanente)
        let saldoActualPlan = parseFloat(credito.saldo_capital); // Fallback
        if (data.proxima_cuota_plan) {
             saldoActualPlan = parseFloat(data.proxima_cuota_plan.saldo_proyectado) + parseFloat(data.proxima_cuota_plan.capital_programado);
        }
            ...
            <div class="card animate-fade-in max-w-2xl mx-auto border-t-4 border-green-600">
                ...
                <div class="grid grid-cols-2 gap-4 mb-6 bg-gray-50 p-4 rounded border">
                    <div>
                        ...
                        <p class="text-2xl font-bold text-blue-800" id="displaySaldo">${formatCurrency(saldoActualPlan)}</p>
                    </div>
                    <div class="text-right">
                ...
                <form id="formPago" onsubmit="guardarPago(event)">
                    ...
                    <!-- DATOS OCULTOS PARA CÁLCULO (USANDO SALDO DEL PLAN) -->
                    <input type="hidden" id="hidden_saldo_capital" value="${saldoActualPlan}">
                    ...
                    <input type="hidden" id="hidden_comision_pendiente" value="${comisionPendiente}">
                    <input type="hidden" id="hidden_fecha_ultimo" value="${fechaUltimoPago}">
                    <input type="hidden" id="maximo_pagable_hidden" value="0">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                        <div class="input-group">
                            ...
                            <input type="date" id="fecha_pago_real" class="w-full p-2 border rounded font-bold" 
                                   value="${fechaSugerida}" required onchange="recalcularInteresAlDia()">
                        ...
                        <div class="input-group">
                            ...
                            <input type="number" id="monto_pagado" class="w-full p-2 border-2 border-green-500 rounded font-bold text-xl" 
                                   step="0.01" value="${montoSugerido}" oninput="simularDistribucion()" required>
                            
                            <!-- Botón Liquidar -->
                            <button type="button" onclick="liquidarDeuda()" class="text-xs text-blue-600 underline font-bold mt-1 hover:text-blue-800">
                                <i class="fas fa-check-double"></i> Pagar Todo (Liquidar)
                            </button>
                            <p class="text-xs text-gray-500 mt-1" id="msg_limite"></p>
                        </div>
                    </div>
                    ...
                    <div class="bg-blue-50 p-4 rounded mb-6 border border-blue-100">
                        ...
                        <div class="grid grid-cols-4 gap-2 text-center text-sm">
                            <div class="bg-white p-2 rounded shadow-sm">
                                <span class="block text-xs text-gray-500">1. Int. Pendiente</span>
                                <strong class="text-orange-600" id="sim_pendiente">$0.00</strong>
                            </div>
                            <div class="bg-white p-2 rounded shadow-sm">
                                <span class="block text-xs text-gray-500">2. Int. Periodo</span>
                                <strong class="text-blue-600" id="sim_periodo" data-value="0">$0.00</strong>
                            </div>
                            <div class="bg-white p-2 rounded shadow-sm">
                                <span class="block text-xs text-gray-500">3. Comisión</span>
                                <strong class="text-purple-600" id="sim_comision">$0.00</strong>
                            </div>
                            <div class="bg-white p-2 rounded shadow-sm border-l-4 border-green-500">
                                <span class="block text-xs text-gray-500">4. Capital</span>
                                <strong class="text-green-700 text-lg" id="sim_capital">$0.00</strong>
                            </div>
                        </div>
                        <div class="mt-2 text-right">
                            <span class="text-xs font-bold text-gray-600">Total Cargos a Cubrir: </span>
...
function recalcularInteresAlDia() {
    ...
    // Recalcular Máximo Pagable (Incluyendo Comisión)
    const intPendiente = parseFloat(document.getElementById('hidden_interes_pendiente').value) || 0;
    const comision = parseFloat(document.getElementById('hidden_comision_pendiente').value) || 0;
    
    const maximoTotal = saldoCapital + intPendiente + interesPeriodo + comision;
    
    document.getElementById('maximo_pagable_hidden').value = maximoTotal.toFixed(2);
    document.getElementById('msg_limite').textContent = `Máximo a pagar (Liquidación): ${formatCurrency(maximoTotal)}`;

    simularDistribucion();
}

function liquidarDeuda() {
    const maximo = document.getElementById('maximo_pagable_hidden').value;
...
function simularDistribucion() {
    ...
    const maximoPagable = parseFloat(document.getElementById('maximo_pagable_hidden').value) || 0;
    ...
    const totalCargos = intPendiente + intPeriodo + comision;
    ...
    // 3. Comisión (NUEVO)
    let pagoComision = 0;
    if (remanente >= comision) {
        pagoComision = comision;
    } else {
        pagoComision = remanente;
    }
    remanente -= pagoComision;
    
    // 4. Capital
    let pagoCapital = remanente;
    ...
    document.getElementById('sim_comision').textContent = formatCurrency(pagoComision);
    document.getElementById('sim_capital').textContent = formatCurrency(pagoCapital);
    document.getElementById('sim_total_interes').textContent = formatCurrency(totalCargos);
...
function renderBadge(est) {
    const map = { 'activo': 'bg-green-100 text-green-800', 'mora': 'bg-red-100 text-red-800', 'solicitud': 'bg-yellow-100 text-yellow-800', 'incobrable': 'bg-gray-800 text-white', 'refinanciado': 'bg-purple-100 text-purple-800' };
